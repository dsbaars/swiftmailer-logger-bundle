...
class RegisterSendListenerCompilerPass implements CompilerPassInterface
{
    ...
    public function process(ContainerBuilder $container)
    {
        ...
        if ($container->hasParameter('tweedegolf_swiftmailer_logger.swift_instances')
            ...
            foreach ($instances as $instance) {
                if ($container->hasDefinition('swiftmailer.mailer.' . $instance)) {
                    ...
                    $def->addMethodCall(
                        'registerPlugin',
                        array(
                        new Reference('tweedegolf_swiftmailer_logger.send_listener')
                        )
                    );
                    unset($def);
                }
            }
        } elseif ($container->hasDefinition('swiftmailer.mailer.default')) { // default instance only
